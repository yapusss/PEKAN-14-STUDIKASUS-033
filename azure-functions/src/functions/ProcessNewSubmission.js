...
app.storageBlob("ProcessNewSubmission", {
  path: "container-studikasus-033/{name}",
  ...
  handler: async (blob, context) => {
    ...
    if (parts.length < 2) {
      context.log(
        `Format nama file '${blobName}' tidak sesuai. Minimal mengandung satu karakter underscore (NIM_Nama_tugas1.ext)`,
      );
      return;
    }
    ...
    try {
      ...
      const sql =
        "UPDATE submissions SET status = 'Submitted' WHERE nim = ? AND status = 'Pending'";
      const [result] = await connection.execute(sql, [nim]);

      if (result.affectedRows > 0) {
        context.log(
          `SUCCESS: Status pengumpulan mahasiswa NIM ${nim} berhasil diubah ke 'Submitted'.`,
        );
      } else {
        context.log(
          `WARNING: Tidak ditemukan data submissions berstatus 'Pending' untuk NIM ${nim}.`,
        );
      }
    } catch (err) {
